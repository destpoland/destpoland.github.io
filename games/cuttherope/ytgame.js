(function() {
    ...
    function l(a) {
        console.log("Passed to l:", a);  // Log to check what 'a' is
        function b(d) {
            return a.next(d);  // Calls the next method on the generator
        }
    
        function c(d) {
            return a.throw(d);  // Calls the throw method on the generator
        }
    
        return new Promise(function(d, e) {
            function f(g) {
                console.log("Generator state:", g);  // Log generator state
                if (g.done) {
                    d(g.value);  // Resolve with the generator's value
                } else {
                    Promise.resolve(g.value).then(b, c).then(f, e);  // Continue generator
                }
            }
            f();  // Start the generator
        });
    }
...
*/
    ...
    ;var Ke = class {
        ...
        saveData(a) {
            const b = this;
            localStorage.setItem("cutRopeSave", JSON.stringify(a));
            return l(function*() {
                if (b.h) {
                    try {
        ...
        loadData() {
            ...
            return l(function* () {
                console.log("Entering generator function");  // Log entry to generator
        
                const localSave = localStorage.getItem("cutRopeSave");
                if (localSave) {
                    try {
                        console.log("Data from localStorage:", localSave);
                        const parsed = JSON.parse(localSave);
                        console.log("Parsed data from localStorage:", parsed);
                        yield parsed;  // Yield the parsed data from localStorage
                    } catch (e) {
                        console.warn("Failed to parse local save:", e);
                    }
                }
        
                // If no saved data, yield default data
                console.log("No save found, yielding default data.");
                yield { level: 1, score: 0 };  // Default fallback data
            });
        }
        firstFrameReady() {
