...
var FileLoader = {
    ...
    request: function(url, method, responseType, currentAttempt) {
        ...
        if (typeof responseType === 'undefined') throw TypeError("No responseType specified");
...
// Helper function to load a script from the response safely
function loadScriptFromResponse(response) {
...
var EngineLoader = {
    ...
    loadAndRunScriptAsync: function(src) {
        FileLoader.load(src, "text",
            ...
            function(error) { throw error; },
            function(response) {
                loadScriptFromResponse(response); // Safely load the script from response
            },
            function(loadedDelta, currentAttempt){
                ProgressUpdater.updateCurrent(-loadedDelta);
            });
